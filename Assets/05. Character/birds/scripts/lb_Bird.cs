...
public class lb_Bird : MonoBehaviour
{
	...
	void OnEnable()
	{
		...
		animator.SetFloat("IdleAgitated", agitationLevel);
		if (dead)
		{
			Revive();
		}
	...
	IEnumerator FlyToTarget(Vector3 target)
	{
		...
		animator.SetBool(landingBoolHash, false);

		// while(anim.GetCurrentAnimatorStateInfo(0).nameHash != flyAnimationHash){	// warning fix
		...
		if (vectorDirectionToTarget.y > .5f)
		{
			...
			while (vectorDirectionToTarget.y > .5f)
			{
				...
				animator.SetFloat(flyingDirectionHash, FindBankingAngle(transform.forward, vectorDirectionToTarget));
		...
		animator.SetFloat(flyingDirectionHash, 0);
		//initiate the landing for the bird to finally reach the target
		Vector3 vel = Vector3.zero;
		flying = false;
		landing = true;
		solidCollider.enabled = false;
		animator.SetBool(landingBoolHash, true);
	...
	{
		// idle = anim.GetCurrentAnimatorStateInfo(0).nameHash == idleAnimationHash;	// warning fix
		idle = animator.GetCurrentAnimatorStateInfo(0).fullPathHash == idleAnimationHash;
		...
		if (idle)
		{
			...
			if (Random.value < Time.deltaTime * .33)
			{
				...
				animator.SetFloat("IdleAgitated", Random.value);
			}
	...
	void DisplayBehavior(birdBehaviors behavior)
	{
		...
		switch (behavior)
		{
			case birdBehaviors.sing:
				animator.SetTrigger(singTriggerHash);
				break;
			case birdBehaviors.ruffle:
				animator.SetTrigger(ruffleBoolHash);
				break;
			case birdBehaviors.preen:
				animator.SetTrigger(preenBoolHash);
				break;
			case birdBehaviors.peck:
				animator.SetTrigger(peckBoolHash);
				break;
			case birdBehaviors.hopForward:
				animator.SetInteger(hopIntHash, 1);
				break;
			case birdBehaviors.hopLeft:
				animator.SetInteger(hopIntHash, -2);
				break;
	...
	void Flee()
	{
		if (!dead)
		{
			StopCoroutine("FlyToTarget");
			GetComponent<AudioSource>().Stop();
			animator.Play(flyAnimationHash);
	...
	void ResetHopInt()
	{
		animator.SetInteger(hopIntHash, 0);
	}
